...
export default {
  ...
  methods: {
    onSubmit (evt) {
      ...
      axios({
        method: 'put',
        url: 'http://localhost:8088/accounts/update/' + that.id,
        data: {
          id: that.id,
      ...
      }).then(response => console.log(JSON.stringify(response)), alert('Account has been updated'), this.$router.push('Accounts')).catch(e => console.log(e))
    },
    onReset (evt) {
      evt.preventDefault()
      this.form.firstName = this.account.firstName
      this.form.lastName = this.account.lastName
      this.form.accountNo = this.account.accountNumber
      this.show = false
    }
  },
  mounted () {
    let self = this
    axios.get('http://localhost:8088/accounts/' + self.id)
      .then(response => {
        this.account = response.data; console.log(this.account)
        this.form.firstName = this.account.firstName
        this.form.lastName = this.account.lastName
        this.form.accountNo = this.account.accountNumber
      })
